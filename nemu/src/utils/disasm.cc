...
extern "C" void disassemble(char *str, int size, uint64_t pc, uint8_t *code, int nbyte) {
  MCInst inst;//存储返回反汇编的指令信息
  llvm::ArrayRef<uint8_t> arr(code, nbyte);
  uint64_t dummy_size = 0;
  gDisassembler->getInstruction(inst, dummy_size, arr, pc, llvm::nulls());//把指令 存储到inst中

  std::string s;
  raw_string_ostream os(s);//存储反汇编的结果输出到s中
  gIP->printInst(&inst, pc, "", *gSTI, os);//输出反汇编的语句
